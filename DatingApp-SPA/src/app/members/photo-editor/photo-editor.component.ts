...
export class PhotoEditorComponent implements OnInit {
  ...
  initializeUploader() {
    ...
    this.uploader.onSuccessItem = (item, response, status, headers) => {
      ...
      if (response) {
        const res: Photo = JSON.parse(response);  // returns response to a JSON object
        // build response as a "photo" from the server
        const photo = {
          id: res.id,
          url: res.url,
  ...
  setMainPhoto(photo: Photo) {
    ...
    this.userService.setMainPhoto(this.authService.decodedToken.nameid, photo.id)
      .subscribe(() => {
        // return copy of filter array where photo isMain is true
